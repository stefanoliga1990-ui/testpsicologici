...
public class RecommendedReadingCatalogue {
    ...
    private final AmazonProductImageProvider imageProvider;
    ...
    public RecommendedReadingCatalogue(AmazonProductImageProvider imageProvider) {
        this.imageProvider = imageProvider;
    }

    public List<RecommendedReading> findByTestId(String testId) {
        List<RecommendedReading> readings = readingsByTestId.getOrDefault(testId, List.of());
        if (readings.isEmpty()) {
            return readings;
        }
        var images = imageProvider.findImages(readings.stream().map(RecommendedReading::asin).toList());
        return readings.stream()
                .map(reading -> reading.withImage(images.get(reading.asin())))
                .toList();
    }
}
